		 ...
		 */
		 init: function(options) {
			 ...
			 if (!form.data('jqv') || form.data('jqv') == null ) {
				...
				//点击图标显示隐藏的文字
				$(".formErrorArrow").live("click", function() {
					 var $con =  $(this).siblings(".formErrorContent");
					if($con.is(":hidden")){
						$con.show();
					}else{
						$con.hide();
			...
			if (options.showArrow) {
				...
				switch (positionType) {
					...
					case "bottomRight":
						...
						arrow.addClass("formErrorArrowBottom").html('<img src="../img/error-icon.png" />');
						break;
					case "topLeft":
					case "topRight":
						arrow.html('<img src="../img/error-icon.png" />');
						prompt.append(arrow);
						break;
					case "centerLeft":
					case "centerRight":
		...
		_calculatePosition: function (field, promptElmt, options) {
			...
			if(!methods.isRTL(field))
			{
				switch (positionType) {
					...
					case "centerRight":
						if (overflow) {
							promptTopPosition = 0;//设置上下居中
							marginTopSize = 0;
							promptleftPosition=field.outerWidth(1)-promptWidth;
						} else {
							promptleftPosition+=field.outerWidth(1)-promptWidth;
							promptTopPosition += field.outerHeight()/2;
						}
						break;
					case "centerLeft":
